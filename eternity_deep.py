"""
eternity_deep.py — Continuous training loop with adaptive topology strategy.

Identical to eternity.py with two key differences:

  1. EXPLORE phase (default): heavily favours ADD_NODE / ADD_CONN / ADD_LAYER to build
     deeper networks from the start (ADD_NODE weight 4x, ADD_LAYER weight 3x).
     SELECTION_POWER is lowered to 2 so structural mutants survive long enough to prove value.

  2. EXPLOIT phase (when stuck): after SWITCH_TO_EXPLOIT_AFTER consecutive evo failures the
     mutation weights shift to heavy weight/bias tuning (MOD_WEIGHT 6x, MOD_BIAS 5x) with
     no structural changes. Switches back to EXPLORE automatically after any improvement.
...
Phase 2 · Evolution
    ...
    Check: median(median_score of last 5 gens) >= prev_run_median x 1.05.
...
from evolution import sortPopulation, getOffspring, mutatePopulation, createPopulation, MutationType
from constants import config, apply_scoring_preset
from agent import Agent
from profiles import get_profile, profile_input_size
import evolution as _evo
...
WEIGHTS_EXPLORE = {
    MutationType.ADD_NODE        : 4,   # was 1 — much more node creation
    MutationType.REMOVE_NODE     : 1,
    MutationType.ADD_CONN        : 3,   # was 2
    MutationType.REMOVE_CONN     : 1,
    MutationType.MOD_WEIGHT      : 2,   # was 4 — less weight tweaking during growth
    MutationType.MOD_WEIGHT_LARGE: 2,   # was 1
    MutationType.MOD_BIAS        : 2,   # was 3
    MutationType.MOD_ACTIVATION  : 2,
    MutationType.SWAP_NODES      : 1,
    MutationType.ADD_LAYER       : 3,   # insert a full hidden layer at once
}

# EXPLOIT: when stuck — fine-tune weights, no structural noise
WEIGHTS_EXPLOIT = {
    MutationType.ADD_NODE        : 1,
    MutationType.REMOVE_NODE     : 0,
    MutationType.ADD_CONN        : 2,
    MutationType.REMOVE_CONN     : 0,
    MutationType.MOD_WEIGHT      : 6,   # was 4 — dominant
    MutationType.MOD_WEIGHT_LARGE: 2,
    MutationType.MOD_BIAS        : 5,   # was 3 — dominant
    MutationType.MOD_ACTIVATION  : 1,
    MutationType.SWAP_NODES      : 2,
    MutationType.ADD_LAYER       : 0,
}
...
def _apply_mutation_mode(mode: str):
    """Overwrite evolution.MUTATION_WEIGHTS with the chosen preset.
    Called in the main process before mutatePopulation; safe because workers
    only evaluate brains, they never call mutatePopulation.
    """
    weights = WEIGHTS_EXPLORE if mode == "explore" else WEIGHTS_EXPLOIT
    _evo.MUTATION_WEIGHTS.update(weights)
...
def main():
    ...
    print(f"Eternity-DEEP runner started  —  output folder: {ETERNITY_DIR}")
    ...
    try:
      while True:
        ...
        while failure_count < MAX_FAILURES:
            ...
            # Adaptive mutation mode: EXPLORE until stuck, then EXPLOIT
            new_mode = "exploit" if failure_count >= SWITCH_TO_EXPLOIT_AFTER else "explore"
            if new_mode != mutation_mode:
                mutation_mode = new_mode
                _apply_mutation_mode(mutation_mode)
                icon = "[EXPLOIT]" if mutation_mode == "exploit" else "[EXPLORE]"
                print(f"  {icon} Mutation mode -> {mutation_mode.upper()} (fail={failure_count}/{SWITCH_TO_EXPLOIT_AFTER})")

            _sub(
                f"Evolution round #{evo_round}  [{mutation_mode.upper()}]  |  "
            ...
            if new_median >= target:
                ...
                failure_count           = 0
                # Switch back to structural exploration after any improvement
